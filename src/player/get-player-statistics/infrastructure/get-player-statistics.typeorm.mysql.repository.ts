  ...
  async getBattleStatistics(userId: string): Promise<BattleStatistics> {
    const ormStats: {
      ...
      await this.battleRepository.query(
        ...
        select
          count(*) as battles,
          sum(a.abandoned = 0 and a.enemy_defense = 0 and a.enemy_health = 0) as victories,
          sum(a.abandoned = 1 or (a.avatar_defense = 0 and a.avatar_health = 0)) as defeats
        from (
          select
            b.id,
            b.abandoned,
          ...
          where
            u.id = ?
            and (b.enemy_defense = 0 and b.enemy_health = 0) or (b.avatar_defense = 0 and b.avatar_health = 0) or (b.abandoned = 1)
        ) as a;
      `,
        [userId],
      )
    ...
    const ormStats: {
      ...
      await this.battleRepository.query(
        ...
        select
          ...
          sum(a.difficulty_id = '64348b20-6e73-4b10-804e-cf212565ac73') as easyTotal,
          sum(a.difficulty_id = '64348b20-6e73-4b10-804e-cf212565ac73' and a.solved = 1) as easySolved,
          sum(a.difficulty_id = '64348b20-6e73-4b10-804e-cf212565ac73' and a.solved = 0) as easyFailed,
          sum(a.difficulty_id = '3b719e85-3eb5-46ba-b641-e21018299b77') as intermediateTotal,
          sum(a.difficulty_id = '3b719e85-3eb5-46ba-b641-e21018299b77' and a.solved = 1) as intermediateSolved,
          sum(a.difficulty_id = '3b719e85-3eb5-46ba-b641-e21018299b77' and a.solved = 0) as intermediateFailed,
          sum(a.difficulty_id = 'bce74b84-995f-49d9-acf3-798cf8847682') as advancedTotal,
          sum(a.difficulty_id = 'bce74b84-995f-49d9-acf3-798cf8847682' and a.solved = 1) as advancedSolved,
          sum(a.difficulty_id = 'bce74b84-995f-49d9-acf3-798cf8847682' and a.solved = 0) as advancedFailed
        from (
          select 
            bmp.id, 
            bmp.solved,
          ...
          from battle_math_problem bmp 
          join battle b on b.id =bmp.battle_id 
          join player_unlocked_math_topic_level pumtl on pumtl.id =b.player_unlocked_math_topic_level_id 
          join player p on p.id = pumtl.player_id 
          join \`user\` u on u.id = p.user_id 
          join math_problem mp on mp.id =bmp.math_problem_id 
          join difficulty d on d.id =mp.difficulty_id 
          where
            u.id = ?
            and (b.enemy_defense = 0 and b.enemy_health = 0) or (b.avatar_defense = 0 and b.avatar_health = 0) or (b.abandoned = 1)
        ) as a;
    ...
    const ormStats: [
      ...
      `
        ...
        from (
          ...
          join \`user\` u on u.id = p.user_id 
          join math_problem mp on mp.id =bmp.math_problem_id 
          join math_topic mt on mt.id =mp.math_topic_id 
          join math_area ma on ma.id =mt.math_area_id 
          where
            u.id = ?
            and (b.enemy_defense = 0 and b.enemy_health = 0) or (b.avatar_defense = 0 and b.avatar_health = 0) or (b.abandoned = 1)
        ) as a
        group by a.math_area_id;
      `,
      [userId],
    );
